"""Consolidate partitioned daily K-line files into flat per-stock parquet files.

Problem: 461K tiny files in daily/{year}/{month}/{code}.parquet (20 rows each).
Fix: Read all partitions per stock -> concat -> sort -> single {code}.parquet.

Run: PYTHONPATH=. ./.venv/Scripts/python scripts/_consolidate_daily.py
"""
import logging
import os
import shutil

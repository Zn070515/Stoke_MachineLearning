"""Phase 3: Transformer training with PyTorch Lightning."""
import argparse
import logging
import os
import shutil
...
def main():
    ...
    logger.info("Training Transformer on %d stock(s)", len(codes))
    ...
    for code in codes:
        ...
        logger.info("  Data: X=%s y=%s n_features=%d", X.shape, y.shape, n_features)
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(folds):
            ...
            checkpoint_cb = pl.callbacks.ModelCheckpoint(
                dirpath=os.path.join(output_dir, "transformer_checkpoints"),
        ...
        if all_val_mcc:
            ...
            if best_path:
                model_path = os.path.join(
                    output_dir, f"transformer_{code}_final.ckpt"
                )

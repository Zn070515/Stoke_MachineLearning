...
Usage:
  PYTHONPATH=. ./.venv/Scripts/python scripts/_backfill_kline_and_turnover.py
  PYTHONPATH=. ./.venv/Scripts/python scripts/_backfill_kline_and_turnover.py --stocks 000001,600519
  PYTHONPATH=. ./.venv/Scripts/python scripts/_backfill_kline_and_turnover.py --dry-run
"""
import argparse
import logging
import os
...
def main():
    ...
    for i, (code, min_existing, max_existing) in enumerate(todo):
        ...
        try:
            ...
            # 3. Ensure same columns, then concat
            existing_cols = ["date", "open", "high", "low", "close",
                             "volume", "amount", "turnover", "pct_change", "stock_code"]
            existing_for_concat = existing[[c for c in existing_cols if c in existing.columns]].copy()
            for col in existing_cols:
                if col not in existing_for_concat.columns and col in bs_backfill.columns:
                    existing_for_concat[col] = np.nan

            merged = pd.concat([bs_backfill, existing_for_concat], ignore_index=True)
            merged = merged.sort_values("date").reset_index(drop=True)
            merged = merged.drop_duplicates(subset="date", keep="last")  # prefer existing data

            # Fill turnover gaps with 0
            ...
            min_d = merged["date"].min().strftime("%Y-%m-%d")
            max_d = merged["date"].max().strftime("%Y-%m-%d")
            has_t = "turnover" in merged.columns and merged["turnover"].notna().sum() > 0
            logger.info("[%d/%d] %s: %d rows [%s → %s] +%d backfill, turnover=%s",
                        i + 1, len(todo), code, len(merged), min_d, max_d,

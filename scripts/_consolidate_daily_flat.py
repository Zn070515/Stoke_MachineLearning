"""Consolidate daily K-line into flat daily/{code}.parquet (non-destructive).

Merges flat daily/{code}.parquet with partitioned daily/{year}/{month}/{code}.parquet
(incremental + backfill), de-duping on date with partition data winning, sorts by date,
and writes back to the flat file. Keeps ALL columns present in source (union), so
e.g. turnover survives. Does NOT delete partition files (safe/reversible).

Usage:
  PYTHONPATH=. ./.venv/Scripts/python scripts/_consolidate_daily_flat.py
  PYTHONPATH=. ./.venv/Scripts/python scripts/_consolidate_daily_flat.py --shard 0/8
  PYTHONPATH=. ./.venv/Scripts/python scripts/_consolidate_daily_flat.py --dry-run
"""
import argparse
import glob
import logging

...
Usage:
  ...
  PYTHONPATH=. ./.venv/Scripts/python scripts/production/build_features.py --no-guba --no-comment
  PYTHONPATH=. ./.venv/Scripts/python scripts/production/build_features.py --jobs 8 --shard 0/4
"""
import argparse
import logging
import os
...
def main():
    ...
    # §十一-3: hash the FULL feature-affecting config.yaml sections (features /
    # preprocessing / universe / fundamental), not a flat subset, so a
    # same-commit config edit invalidates the feature cache.
    worker_args["_git_commit"] = cache_manifest.git_head()
    worker_args["_config_hash"] = cache_manifest.config_hash(
        cache_manifest.config_snapshot(cfg)
    )
    ...
    if args.quality_gate:
        ...
        # §六-1: the gate must review the SAME data root this build wrote and
        # require the datasets this mode produced — a gate that PASSes on a

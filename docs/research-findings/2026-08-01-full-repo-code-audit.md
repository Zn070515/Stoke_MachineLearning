...
8. **16 pre-existing test failures + 3 guba network errors (new, from Phase G verification).** Full-suite run at HEAD: 207 passed, 23 failed, 4 errors. Classification: (a) 4 outlier failures = audit regressions, fixed (28dc5a2); (b) 3 guba_source network errors = environmental (WAF blocking), not code; (c) 16 failures that ALSO fail at the pre-audit baseline (6335c7b) with data present — root cause for many is the macro merge + blanket `dropna()` in `_prep_feature_df`: merging `macro_daily.parquet` injects ~180-200 NaN rows per `gdp_cn_yoy_z20`-style column, then the blanket `dropna()` empties the frame. Outside the original audit scope; candidates for follow-up work. **RESOLVED (fix commit after 954d6d4):** the 16 were a mix of (1) code bugs — `zscore` returning NaN for constant windows (quarterly macro forward-filled) wiped frames via `_prep_feature_df`'s blanket `dropna()` (now returns 0), `sliding_window_view` crashing on short inputs in `technical.py` Alpha158 factors (now guarded `len < d`), `CrossSectionNormalizer` default stages diverging from config (`["sector","size","rank"]` → `["sector","size","adaptive"]`), `RobustScaler`'s `nan_to_num(nan=0.0)` zeroing genuine warmup NaN (removed; clip handles inf) — and (2) stale tests: 3-class labels expected binary ({0,1}), `PanelDataset` tests unpacked 6-tuple before `date_idx` was added, cross-section sector/size tests assumed 1-stock sectors and a nonexistent `market_cap`/`x_raw` column, and two `FeatureRegistry` queries asserted `f2` was excluded despite matching. Full suite minus the deselected guba_source network tests: 224 passed, 0 failed.

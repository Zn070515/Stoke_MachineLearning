...
> **Status:** Findings catalog. Fix plan tracked in `docs/superpowers/plans/2026-08-01-full-repo-audit-fixes.md`.
...
### P0-1. Dividend yield systematically 10× inflated (full dataset)
...
### P0-2. Kalman medium-gap fill injects post-gap (future) data
...
**P1-1. InteractionFeatures.compute_all runs BEFORE aux merge → interaction dimension always empty**
...
**P1-2. Static features `daily_ret_vol`/`daily_ret_mean` use ENTIRE history (incl. test) → look-ahead**
...
**P1-3. Date-range stitch concatenates incompatible volume units (手 vs 股, 100×)**
...
**P1-4. Failover backfill path crashes with TypeError**
...
**P1-5. MarketWideStorage.save dedups on ['date'] only → collapses multi-row-per-day events**
...
**P1-6. Fundamental forward_fill_to_daily final .ffill() defeats max_gap_days**
...
**P1-7. skip_completed_stocks deletes raw file whenever oldest date doesn't reach start_date**
...
**P1-8. skip_completed_years uses `<=` → skips current year forever**
...
**P1-9. news_pipeline.fetch_bodies parameter ignored**
...
**P1-10. Tencent minute source: 8 columns declared, 6 fields per row → crash, source dead**
...
**P1-11. cninfo_source._query_page swallows exceptions → returns [],False → permanent pagination stop**
...
**P1-12. SectorBroadcaster invoked per-stock → cross-sectional features collapse to 0**
...
**P1-13. FlowDecomposer `flow_alpha_residual`/`flow_price_divergence` never produced**
...
**P1-14. Event-time features map raw event dates with `get_indexer(method="nearest")` → ties pick EARLIER trading day**
...
**P1-15. Lockup `unlock_return_30d` uses `shift(-30)` on sparse per-event rows + forward-fill → leak/wrong**
...
**P1-16. WalkForwardSplitter.purge_days defaults to 0**
...
**P1-17. download_market_data.py northbound concurrent path missing None guard**
...
| P2-13 | `stoke_ml/features/pipeline.py:1203` | `aligned_close` holds consecutive closes → horizon>1 metrics wrong holding period |
...
| P2-16 | `stoke_ml/preprocessing/config.py:191` | `persistence_mode`/`event_time_features`/`persistence_halflife` are DEAD config (never read) |
| P2-17 | `stoke_ml/preprocessing/config.py:233` | `monitor:`/`registry:` sections entirely dead (never instantiated) |

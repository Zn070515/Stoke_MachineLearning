...
> 2026-07-27 v5.2 | 50 个目录/文件逐一手工递归审计 | 5,530 只 A 股 | 5 项质量指标 × 53 个数据维度
>
> **v5.2 更新（2026-07-27 凌晨）：分钟线 + shareholder 关键缺口已补**
> 1. minute/5min 1,890→5,201 (+175%)：Sina 直连 HTTP 源替代 AKShare V8，5 分片并行 0.3s/只，~9 分钟完成 3,311 只补全
> 2. minute/30min 也已满覆盖至 5,201（SinaDirect 源），minute/15min 满覆盖至 5,201（Tencent mkline 源）
> 3. 新增两个纯 HTTP 分钟线数据源：`SinaDirectMinuteSource`（无 V8 依赖，0.3s rate limit）+ `TencentMinuteSource`（独立腾讯服务器）
> 4. `download_minute.py` 新增 `--source`（akshare/sina-direct/tencent）和 `--resume` 参数
> 5. shareholder 确认为季度时间序列：5,486 只 × 53 季度（2013Q1～2026Q3），评分从单快照 A(4.2)→季度时序 A+(4.8)
>
...
| 3 | `minute/5min` | 5,201 | ~1,970 | 9 | 2026-05~07 (2mo) | 5 | 5 | 3 | 5 | 5 | **4.6** | A+ |
| 4 | `minute/15min` | 5,201 | ~320 | 9 | 2026-06~07 (1mo, Tencent源) | 5 | 5 | 3 | 5 | 5 | **4.6** | A+ |
| 5 | `minute/30min` | 5,201 | ~1,971 | 9 | 2025-07~2026-07 (1yr) | 5 | 5 | 4 | 5 | 5 | **4.8** | A+ |
| 6 | `minute/60min` | 5,200 | ~1,970 | 9 | 2024-07~2026-07 (2yr) | 5 | 5 | 4 | 5 | 5 | **4.8** | A+ |

> **v5.2 注：** minute 四频率全部满覆盖（5,200+ 只，94%+）。5min 从 v5 的 1,890→5,201（+175%），15min 从 4,564→5,201（+14%），via 两个新数据源（SinaDirect HTTP + Tencent mkline）。Tencent 15min 仅 320 bar 历史深度（~1 月），后续如需更深历史可重新跑 SinaDirect 覆盖。minute 列 = `[datetime, open, high, low, close, volume, stock_code, bar_period]`（SinaDirect 无 amount 列）。
...
| 35 | `shareholder` | 5,486 | ~53 | 6 | 2013-Q1 ~ 2026-Q3 (54季度) | 5 | 5 | 5 | 5 | 4 | **4.8** | A+ |
| 36 | `shareholder_processed` | — | — | — | 待重新处理为日频 | — | — | — | — | — | **—** | 待处理 |
| 37 | `pledge` | 3,259 | ~72 | 16 | — | 3 | 4 | 3 | 5 | 4 | **3.8** | B+ |

> **v5.2 更新 (shareholder)：** 已从 `RPT_HOLDERNUMLATEST`（单快照）切换到 `RPT_HOLDERNUM_DET`（全季度历史）。5,486 只 × 53 行/只（2013Q1～2026Q3），列 = `[date, stock_code, holder_num, change_num, change_ratio, avg_shares]`。时效性从 2→5。`shareholder_processed` 待用新数据重新前向填充到日频。
>
...
| 等级 | v5.1 数量 | **v5.2 数量** | 变化 | 代表维度 |
|------|----------|--------------|------|----------|
| **A+** (4.5-5.0) | 23 | **25** | +2 | +minute/5min/15min, +shareholder |
| **A** (4.0-4.4) | 20 | **17** | −3 | minute 升 A+, shareholder 升 A+, shareholder_processed 待处理 |
| **B+** (3.5-3.9) | 6 | **6** | — | |
| **B** (3.0-3.4) | 5 | **5** | — | |
| **C** (2.0-2.9) | 1 | **1** | — | analyst |
| **F** (<2.0) | 0 | **0** | — | |

> **加权平均（53 个维度）：~4.42 / 5.0 → A+**（v5.1: 4.36，+0.06 来自 minute 两频率满覆盖 + shareholder 季度时序）
>
> v5.2 的改善：
> 1. **minute 四频率全 A+**：5min 1,890→5,201（+175%），15min 4,564→5,201（+14%），两条频率覆盖从最大短板变为满覆盖
> 2. **新增两个纯 HTTP 数据源**：`SinaDirectMinuteSource`（Sina 直连，0.3s/只，无毒 V8）+ `TencentMinuteSource`（腾讯 mkline，独立服务器）
> 3. **shareholder 从单快照到季度时序**：5,486 只 × 53 季度（2013Q1～2026Q3），时效性 2→5
> 4. **`download_minute.py` 升级**：新增 `--source`（akshare/sina-direct/tencent）和 `--resume` 参数
...
| ~~扩展 minute/5min 覆盖~~ ✅ v5.2 | minute/5min | A (4.0) | A+ (4.6) | +0.6 | 已完成 |
| ~~重新下载 shareholder 全季度历史~~ ✅ v5.2 | shareholder | A (4.2) | A+ (4.8) | +0.6 | 已完成 |
| 重新处理 shareholder_processed 为日频 | shareholder_processed | — | A+ (5.0) | — | 低 |
| 长期积累 limit_up_* | limit_up_* | A (4.0) | A+ (4.6) | +0.6 | 低（需时间） |
| 长期积累 concept_blocks | concept_blocks/* | B+/A (3.8/4.0) | A (4.2) | +0.4 | 低（需时间） |
| 用 SinaDirect 覆盖 Tencent 15min（需更深历史时） | minute/15min | A+ (4.6) | A+ (4.8) | +0.2 | 低 |
...
```
原始数据  v2 (07-24)  →  v3 (07-25)  →  v4 (07-25深夜)  →  v5 (07-26)  →  v5.1 (07-26深夜)  →  v5.2 (07-27凌晨)  →  补完剩余缺口
────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
A+ (25): v5.1 全部 +
         minute/5min(4.6)
         minute/15min(4.6)
...
A  (17): v5.1 A 全部 − minute/5min − minute/15min − shareholder − shareholder_processed(待处理)
...
均分: 4.15(A) → 4.32(A) → 4.36(A) → 4.30(A) → 4.36(A) → 4.42(A+) → ~4.55(A+)
```
...
> **2026-07-27 v5.2：** 首次完成 `data/a_shares/` 下 **50 个顶级项目、100% 递归子目录**的手工审计 + **5 项关键修补**。53 个评分维度加权均分 **~4.42/5.0 (A+)**，F 级维度清零。
>
> v5.2 在 v5.1 基础上补上了两个最长的短板：
> - **minute 四频率满覆盖**：5min 1,890→5,201 (+175%)，15min 4,564→5,201 (+14%)，via 两个新 HTTP 源
> - **shareholder 季度时序**：从单快照到 54 季度历史（2013Q1～2026Q3），换用 `RPT_HOLDERNUM_DET` API
> - **新数据源基础设施**：`SinaDirectMinuteSource` + `TencentMinuteSource`，纯 HTTP 无 V8 依赖，`--resume` 断点续传
>
...
| # | 任务 | 影响 | 难度 |
|---|------|------|------|
| 1 | 重新处理 `shareholder_processed` 为日频前向填充 | 解锁筹码集中度变化率特征 | 低 |
| 2 | 长期 cron 积累 `limit_up_*` + `concept_blocks` | 打板/概念板块特征可用 | 低（需时间） |
| 3 | 扩展 `news_raw` 历史 <1年→5年（换 AKShare 新闻接口） | 新闻情绪覆盖完整牛熊周期 | 中 |
